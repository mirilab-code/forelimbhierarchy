...
%
function plotResults_G_ETM_new(dataFile,date,Psi3, parsForPlotting, figuresDir)
...
set(gca, 'FontSize', 8, 'XTick', [1:nNeurons], 'YTick', [1:nNeurons]);
xlabel('source', 'FontSize', 22);
ylabel('target', 'FontSize', 22);
xline(round(num_CFA)+0.5,'r');
yline(round(num_CFA)+0.5,'r');
title('Recovered Connectivity');
if exist('figuresDir', 'var') == true
...
figure('Name', 'Recovered Topology - 10k pairs only');
imagesc(Psi3, [0 1]);
colormap([0 0 0; 0 1 0]);
set(gca, 'FontSize', 8, 'XTick', [1:nNeurons], 'YTick', [1:nNeurons]);
xlabel('source', 'FontSize', 22);
...
xline(round(num_CFA)+0.5,'r');
yline(round(num_CFA)+0.5,'r');
title('Recovered Connectivity - 10k pairs only');
if exist('figuresDir', 'var') == true
    outName = [figuresDir, '/', fName, '_recoveredConnectivity'];
    fprintf('Printing %s\n', outName);
    print('-depsc', outName);
