...
%
% OutStruct is a structure whose fields contain the results of the Grange
% fitting process.
...
%
function GrangerRes = runGranger_G_ETM_only_parneur_bottomhalf(OutStructTop, SpikeTrains, globalRegressor_In, historyRegressor, historyRegressorNBins,pairs_used)
...
beta_Global = OutStructTop.beta_Global;
beta_Global_pVals = OutStructTop.beta_Global_pVals;
beta_History = OutStructTop.beta_History;
beta_History_pVals = OutStructTop.beta_History_pVals;

% here we go with the first round of the causal test
glm_dev = OutStructTop.glm_dev;
% log-likelihood of the data
spikes_LL = OutStructTop.spikes_LL;
% Akaike's information criterion
aic = OutStructTop.aic;
...
%     
% 	for currHistoryRegressorInd = 1:nhistoryRegressorNBins
% 		currNRegressorSteps = historyRegressorNBins(currHistoryRegressorInd);
% 		
% 		parfor currNeuronInd = 1:nNeurons
...
% 			
%             [tmp_Global, tmp_History, dev, tmp_Global_pVals, tmp_History_pVals] = ...
% 				fitGLM_G_ETM(SpikeTrains, globalRegressor, ...
...
% 				computeLogLikelihood_SpikeTrains(SpikeTrains, beta_Global{currNeuronInd, currHistoryRegressorInd, currGlobalRegressorInd}, ...
...
% 			aic(currNeuronInd, currHistoryRegressorInd, currGlobalRegressorInd) = -2*spikes_LL(currNeuronInd, currHistoryRegressorInd, currGlobalRegressorInd) + ...
% 				2*(numel(beta_History{currNeuronInd, currHistoryRegressorInd, currGlobalRegressorInd}) + ...
% 				numel(beta_Global{currNeuronInd, currHistoryRegressorInd, currGlobalRegressorInd}) + 1);
% 		end;
% 	end;
% end;
...
for currNeuronInd = 1:nNeurons
	...
	% The following lines of code are for handling a peculiar
	% behavior of Matlab.
...
for targetNeuronInd = 1:nNeurons
	...
	% here we use Matlab's parallel toolbox
	% if that is not available, substitute the parfor with a simple for
    %%%%% Here we adapt and choose only the target neurons that are
    %%%%% necessary to compute
    temp_var_pairs = find(pairs_used(:,1)==targetNeuronInd);
    triggerNeuronsIndUsed  = pairs_used(temp_var_pairs,2)';
	parfor triggerNeuronInd = 1:nNeurons
        if ismember(triggerNeuronInd,triggerNeuronsIndUsed)
		fprintf('Causal Step - Processing target neuron #%d - -- trigger neuron #%d\n', ...
			targetNeuronInd, triggerNeuronInd);
		fprintf('Target Neuron: nHistoryBins %d -- nGlobalBins %d\n', ...
			nhistoryRegressorNBins, globalRegressor.nBins);
		
		% here we call function for fitting a GLM model to our data
		[tmp_Global, tmp_History, dev, tmp_Global_pVals, tmp_History_pVals] = ...
			fitGLM_G_ETM(SpikeTrains, ...
