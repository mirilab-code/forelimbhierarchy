...
switch dataSet
    ...
    case 3
        D = load('/projects/p31350/Granger_Quest/high_FR_granger_40.mat');
...
globalRegressor.nBins = [1, 5, 10, 13, 20, 25];
...
str1 = 'C:/Users/mirilab/Documents/Adam/Granger_Quest/results/09222020_Out.mat';
plotResults_Granger(str1);
...
conns(2) = length(find(psi(num_CFA+1:end,1:num_CFA)));
conns(3) = length(find(psi(1:num_CFA,num_CFA+1:end)));
conns(4) = length(find(psi(num_CFA+1:end,num_CFA+1:end)));
all_str = ['CFA';'CFA';'CFA';'RFA';'RFA';'CFA';'RFA';'RFA'];
str = sprintf('\n');
disp(str)
