...
xDims_within = {3, 3};    % These numbers match the within-group latents in the synthetic ground truth
...
maxIters = 1e5;           % Limit the number of EM iterations (not recommended for final fitting stage)
...
cd(session_path)
save(sprintf('%s_run%g_dlag_model1.mat', work, runIdx))
...
[seqEst, ~] = exactInferenceWithLL_dlag(pc_struct, res.estParams);
...
% Note that latent variables are not, in general, ordered. So don't try to
% match estimated latent 1 to ground truth latent 1, and so on.
plotDimsVsTime_dlag(pc_struct, 'xsm', trueParams, res.binWidth, ...
                  'nPlotMax', 1, ...
                  'plotSingle', true, ...
                  'plotMean', true, ...
                  'units', []);
...
delaySig = bootstrapDelaySignificance(pc_struct, ...
                                      res.estParams, ...
                                      numBootstrap, ...
                                      'parallelize', parallelize, ...
                                      'numWorkers', numWorkers);
...
bootParams = bootstrapGPparams(pc_struct, ...
                               res.estParams, ...
                               binWidth, ...
                               numBootstrap, ...
                               'alpha', alpha, ...
...
%% Save
cd(session_path)
save(sprintf('%s_run%g_dlag_workspace.mat', work, runIdx))
