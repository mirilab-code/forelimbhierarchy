function [events,data] = ks_to_events(path_to_ks,whichunits)
...
spike_widths = templateDuration;
events_total = [spike_clusters spike_times spikeDepths spikeDepths*0];
% make the unit index start at 1 instead of 0
events_total(:,1) = events_total(:,1)+1;

% change the zero column so that it is the spike widths for each unit
for i=1:size(events_total,1)
    u = events_total(i,1);
    width = spike_widths(u);
    events_total(i,4) = width;
end
...
% create the information structure so you can query a neuron and get it's
% depth and waveform and firing rate etc.
units = unique(events(:,1));
% data = cell(length(units),1);
data = [];
...
for i=1:length(units)
    ...
    u = units(i);
    data(u).unit = u;
    data(u).waveform = waveforms(u,:)';
    
    row = find(events(:,1)==u,1);
    data(u).depth = events(row,3);
    data(u).width = events(row,4);
