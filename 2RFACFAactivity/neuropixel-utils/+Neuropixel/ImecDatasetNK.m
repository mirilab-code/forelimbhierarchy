...
% Author: Daniel J. O'Shea (2019)
    ...
    methods
        function df = ImecDatasetNK(fileOrFileStem, varargin)
            p = inputParser();
            ...
            file = Neuropixel.ImecDatasetNK.findImecFileInDir(fileOrFileStem, 'ap');
            if isempty(file)
                error('No AP Imec file found at or in %s', fileOrFileStem);
            end
            [df.pathRoot, df.fileStem, df.fileTypeAP] = Neuropixel.ImecDatasetNK.parseImecFileNameNK(file);
...
%             df.highPassFilterHz = meta.imHpFlt;
            df.highPassFilterHz = NaN;      % I did this myself
        ...
        function newImec = copyToNewLocation(df, newRoot, newStem)
            ...
            docopy(df.pathAP, f('.imec0.ap.bin'));
            docopy(df.pathAPMeta, f('.imec0.ap.meta'));
            docopy(df.pathLF, f('.imec0.lf.bin'));
            docopy(df.pathLFMeta, f('.imec0.lf.meta'));
            docopy(df.pathSync, f('.imec0.sync.bin'));

            newImec = Neuropixel.ImecDatasetNK(fullfile(newRoot, newStem), 'channelMap', df.channelMapFile);
...
%             prog.finish();
    ...
    methods % Dependent properties
        ...
        function fileAP = get.fileAP(df)
            fileAP = [df.fileStem '.imec0.ap.bin'];
        end
    ...
    methods % Modify bin data files in place
        function modifyAPInPlace(imec, varargin)
            imec0.modifyInPlaceInternal('ap', varargin{:});
        end

        function modifyLFInPlace(imec, varargin)
            imec0.modifyInPlaceInternal('lf', varargin{:});
        end
        ...
        function imecSym = symLinkAPIntoDirectory(imec, newFolder, varargin)
            ...
            imecSym = Neuropixel.ImecDatasetNK(newAPPath, 'channelMap', imec.channelMapFile);
        end
         ...
         function imecOut = saveTranformedDataset(df, outPath, varargin)
            ...
            imecOut = Neuropixel.ImecDatasetNK.writeConcatenatedFileMatchGains({df}, outPath, p.Results);
        end
    end
    ...
    methods(Static)
        ...
        function tf = folderContainsDataset(fileOrFileStem)
            ...
            [pathRoot, fileStem, fileTypeAP] = Neuropixel.ImecDatasetNK.parseImecFileNameNK(file);
        ...
        function file = findImecFileInDir(fileOrFileStem, type)
            ...
            if exist(fileOrFileStem, 'file') == 2
                file = fileOrFileStem;
                [~, ~, type] = Neuropixel.ImecDatasetNK.parseImecFileNameNK(file);
                switch type
                    case 'ap'
            ...
            elseif exist(fileOrFileStem, 'dir')
                ...
                switch type
                    case 'ap'
                        apFiles = Neuropixel.ImecDatasetNK.listAPFilesInDir(path);
                        if ~isempty(apFiles)
                            if numel(apFiles) > 1
                    ...
                    case 'lf'
                        lfFiles = Neuropixel.ImecDatasetNK.listLFFilesInDir(path);
                        if ~isempty(lfFiles)
                            if numel(lfFiles) > 1
                ...
                switch type
                    case 'ap'
                        candidates = Neuropixel.ImecDatasetNK.listAPFilesInDir(parent);
                        
                    case 'lf'
                        candidates = Neuropixel.ImecDatasetNK.listLFFilesInDir(parent);
        ...
        function [pathRoot, fileStem, type] = parseImecFileNameNK(file)
            if iscell(file)
                [pathRoot, fileStem, type] = cellfun(@Neuropixel.ImecDatasetNK.parseImecFileNameNK, file, 'UniformOutput', false);
                return;
            end
            file = char(file);
...
%             match = regexp(file, '(?<stem>\w+).imec0.(?<type>\w+).bin', 'names', 'once');
        ...
        function imecOut = writeConcatenatedFileMatchGains(imecList, outPath, varargin)
            ...
            [parent, leaf, ext] = Neuropixel.ImecDatasetNK.filepartsMultiExt(outPath);
            ...
            % figure out which channels to keep
            [chIndsByFile, ~] = Neuropixel.ImecDatasetNK.multiFile_build_channelSelectors_internal(imecList, 'goodChannelsOnly', p.Results.goodChannelsOnly, ...
            ...
            if ~dryRun
                Neuropixel.ImecDatasetNK.clearDestinationStem(fullfile(outPath, leaf));
            end
            ...
            outFile = fullfile(outPath, [leaf '.imec0.ap.bin']);
            imecOut = Neuropixel.ImecDatasetNK(outFile, 'channelMap', imecList{1}.channelMapFile);
