function failed_reaches = find_failed(num_spouts,reward_times,touch,suction,err)
    ...
    failed_reaches = cell(1,num_spouts);
    success_bounds = cell(1,num_spouts);
    non_reach_total = cell(1,num_spouts);
    for ii = 1:num_spouts
        channel = ii;
%         touch = [touch; suction];
    ...
    %             disp('success!');
            ...
            non_reach_total{ii} = non_reach;
        end
        failed_reaches{ii} = length(reward_times{ii}) - non_reach_total{ii} - size(success_bounds{ii},1);
    end
